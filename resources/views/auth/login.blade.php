...
<html lang="en">
	<head>
		...
		<title>Masuk Pendaftaran Kursus</title>
		<!-- Bootstraps -->
		...
		<div class="container">
            @if (session('fail'))
				<div class="alert alert-danger alert-dismissible fade show" role="alert">
					...
					<button type="button" class="btn-close px-4" data-bs-dismiss="alert" aria-label="Close"></button>
				</div>
			@endif
			<div class="forms-container">
				<form action="{{ route('loginMahasiswa.login') }}" method="POST">
					@csrf
					<h2 class="title">Masuk</h2>
					<p class="sub-title">Silahkan masuk dengan akun anda</p>
					<div class="input-field">
						<i class="fas fa-user"></i>
						<input type="npm" name="npm" autofocus placeholder="NPM" required/>
					</div>
					<div class="input-field">
